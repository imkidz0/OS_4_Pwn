...
3. 커널은 인터럽트([[Interrupt]]), 페이지 폴트([[Page Fault]]), 시스템콜([[System Call]]) 등을 처리하고, 이러한 요청들은 요청이 들어온 즉시 수행하지 않으면 커널이 정상 동작할 수 없게 되어 커널 안정성이 붕괴될 것이다.
