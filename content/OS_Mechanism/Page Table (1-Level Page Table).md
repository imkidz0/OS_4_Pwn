...
해당 글에 대한 이해가 끝났으면 현대의 운영체제가 사용하는 [[4-Level Page Table (Multi-Level Paging)]]에 대해 다룬 문서가 있으니 이어서 읽을 것을 권장한다.
...
이때 `0x800000000e386067` 같은 값들은 가상 페이지를 어떤 물리 프레임에 매핑할지와 접근 권한을 함께 담고 있는 [[Page Table Entry (PTE)]]이다.
...
VPN은 가상 주소가 속한 페이지이고, PFN은 해당 페이지가 올라간 실제 프레임이다.
이때 VPN을 가상 주소라고 혼동하면 안된다.

VPN은 Virtual Page "Number", 즉 가상 주소 공간에서 이게 몇 번째 페이지인지를 나타내는 번호이다.
...
크게 보면 VPN은 가상 주소가 속한 페이지의 번호이고 Offset이 해당 페이지에서의 위치인 것이다. 따라서 `0x00000000004018ab`를 VPN과 Offset으로 변환하게 되면,
...
쉽게 말해 `0x00000000004018ab`안에 저장된 값은 운영체제 입장에서 "0x401 페이지의 0x8ab번째 값"이 되는 것이다.
...
이때, 가상 주소 `0x4018ab`의 VPN은 0x401이므로, Page Table에서 401번째 인덱스의 값이 해당 가상 주소의 PTE가 된다.
...
`Physical Address = (PFN << 12) | Offset`으로 구하고, 이 때 Offset은 가상 주소에서의 Offset인 0x8ab와 동일하다.
...
이렇게 Page Table을 통해 가상 주소 0x4018ab는 물리 주소 0xe3868ab에 매핑된다는 것을 알 수 있다.
이러한 계산 과정은 [[Memory Management Unit (MMU)]]에서 담당한다.