...
- 해당 전역 변수의 가상 주소를 읽는 것이 "커널 물리 메모리의 값을 읽는 것"이 되고,
- 해당 전역 변수의 가상 주소에 쓰는 것이 "커널 물리 메모리에 값을 쓰는 것"이 된다.

즉, PTE를 변조함으로써 **가상 주소 -> 임의의 물리 페이지로 매핑**할 수 있게 되어, 유저 공간에서 **커널 물리 메모리에 대한 Arbitrary R/W**가 가능해진다.
